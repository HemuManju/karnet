...
with skip_run('skip', 'imitation_with_kalman_state') as check, check():
    # Load the configuration
    cfg = yaml.load(open('configs/carnet.yaml'), Loader=yaml.SafeLoader)
    cfg['logs_path'] = cfg['logs_path'] + str(date.today()) + '/IMITATION_KALMAN'
    ...
    net = KalmanStateNet(cfg)
    waypoint, speed = net(
        net.example_input_array, net.example_command, net.example_kalman
    )
...
with skip_run('skip', 'benchmark_kalman_state_imitation') as check, check():
    # Load the configuration
    cfg = yaml.load(open('configs/imitation.yaml'), Loader=yaml.SafeLoader)
    ...
    for exp_id, config in enumerate(all_experiment_configs):
        ...
        # Update the model
        restore_config = {
            'checkpoint_path': f'logs/2023-10-27/IMITATION_KALMAN/imitation_{navigation_type}.ckpt'
        }
